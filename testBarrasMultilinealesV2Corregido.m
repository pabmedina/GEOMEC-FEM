...
while rompedor < 1
    ...
    if count>0
        ...
        if gapViejo>(despRot)*traccionUltima
            if count==1
                direccion=-1;
                count=2;
            end
...
%             K(eleDofs,eleDofs) = K(eleDofs,eleDofs) - Ke;
            ...
            if gap<=despRot
                E = ((((D(3)-D(2))-desp1)*(-desp1*E_00))/(despRot-desp1)+desp1*E_00)/(D(3)-D(2));
                Ediff_d2 = (E_00*desp1 - (E_00*desp1*(D(2) - D(3) + desp1))/(desp1 - despRot))/(D(2) - D(3))^2 + (E_00*desp1)/((D(2) - D(3))*(desp1 - despRot));
                Ediff_d3 = -(E_00*desp1 - (E_00*desp1*(D(2) - D(3) + desp1))/(desp1 - despRot))/(D(2) - D(3))^2 - (E_00*desp1)/((D(2) - D(3))*(desp1 - despRot));               
            else
                E = 0;
            end
