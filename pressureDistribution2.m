function pElementsBarra = pressureDistribution2(vars4solve)
...
qLeakOffMax = 2*9.23E-03;
qLeakOffMin = 2*1.59E-04;
...
%     for iFisura = 1:nFisuras
%         vars4solve.pBC(1 + (iFisura - 1)*nodosPorFisura:(1 + (iFisura - 1)*nodosPorFisura) + 11,:) = p1(iFisura)*ones(nodosPorFisura,2);   %%% Arreglar
%     end
%     qLK0 = pressure4LeakOff(vars4solve);
%     qLK0 = qLK0(:,1)+qLK0(:,2);
%     qLK0 = -qLK0;
%     for j = 1:MAXi
%         [pElementsBarra, qElementsBarra, Qbomba] = solverNFisuras(qLK0, nFisuras, dofsBCcell, QbPoly, p1,elementsBarraT,nodeDofscell,Hcell,nodosPorFisura);
