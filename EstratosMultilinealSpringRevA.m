...
%     while CriterioCV_Presion > 1e-4 %CriterioCV > 1e-3 && CriterioCV_desplazamientos > 1e-3  
    ...
    Pfactor   
    if itime<=25
        Pin         = 5e6 + Pfactor*5e5;%PresioPoralInicial + PresioPoralInicial*0; %40e6;%     % Nacho no mienta0;s : (1e5)*itime+30e6;
    else 
        Pin = 5e6 + Pfactor*5e5;
    end
...
%     graficadorMalladorH8( nodNew, elem8Nod, nodNew(:,3))
    ...
    [TBombas]=activacionDeBombas(nTiempo,3);%(delta_t*9.1*3, 3);% % para secuenciar Decia activacionDeBombas(delta_t*9.1*3, 3) MOD NACHO!!!
