...
despRot = 0.02;  %   0.6; %   %% Definirlo según el módelo de resorte que se elija. Eso va a depender de la distancia al wellbore

E_BB = zeros(nelSpringFisura1,1);
gapViejo = zeros(nelSpringFisura1,1);
desp_1Variable=zeros(nelSpringFisura1,1);
count2 = zeros(nelSpringFisura1,1);
...
%     if iter <= 50
    if itime<=25
        Pin         = 60e6*itime*(1/25);%PresioPoralInicial + PresioPoralInicial*0; %40e6;%     % Nacho no mienta0;s : (1e5)*itime+30e6;
    else 
        Pin         = 60e6-60e6*(itime-25)*(1/50);
    end
%     end
    if iter > 25
    ...
    %        E_variable = E_B;
            ...
            if gap_fisu1(ispring)>=desp_1Variable(ispring) && gap_fisu1(ispring)>=gapViejo(ispring,itime)
                if gap_fisu1(ispring)<=despRot
                    E_variable(ispring) = (((gap_fisu1(ispring)-desp_1Variable(ispring))*(-desp_1Variable(ispring)*E_BB(ispring,itime)))/(despRot-desp_1Variable(ispring))+desp_1Variable(ispring)*E_BB(ispring,itime))/gap_fisu1(ispring);
                    Ediff_d1 = (E_BB(ispring,itime)*desp_1Variable(ispring) - (E_BB(ispring,itime)*desp_1Variable(ispring)*(-gap_fisu1(ispring) + desp_1Variable(ispring)))/(desp_1Variable(ispring) - despRot))/gap_fisu1(ispring)^2 + (E_BB(ispring,itime)*desp_1Variable(ispring))/((-gap_fisu1(ispring))*(desp_1Variable(ispring) - despRot));
                    Ediff_d2 = -(E_BB(ispring,itime)*desp_1Variable(ispring) - (E_BB(ispring,itime)*desp_1Variable(ispring)*(-gap_fisu1(ispring) + desp_1Variable(ispring)))/(desp_1Variable(ispring) - despRot))/(-gap_fisu1(ispring))^2 - (E_BB(ispring,itime)*desp_1Variable(ispring))/((-gap_fisu1(ispring))*(desp_1Variable(ispring) - despRot));   
                else
...
%             if gapViejo(ispring,itime)>=desp_1Variable(ispring) && gap_fisu1(ispring)<=gapViejo(ispring,itime)
            if gapViejo(ispring,itime)>=0 && gap_fisu1(ispring)<=gapViejo(ispring,itime)
...
%     for ispring = 1:nelSpringFisura1
%         if gap_fisu1(ispring)>=despRot
