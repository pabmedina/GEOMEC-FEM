...
malla = 'M22';%'M3';%'M22';%'M22';%'M333P';%'M333P';%'M4';%'M3test';%%;%
...
% wpg = ones(npg,1);
% 
% area.fisura11 = zeros(length(ele_fisu1_y1_1),1);
% for iele = 1:length(ele_fisu1_y1_1)
...
%         dN = dN([1 3],[1 2 5 6]);
%         
%         nodesEle = nodesEle([1 2 5 6],[1 3]);
%         jac = dN*nodesEle;                
%         area.fisura11(iele) = area.fisura11(iele) + det(jac);
%            
%     end
% end
...
for iele = 1:nel
    ...
    for inod = 1:nnodel
        ...
        if j(inod) < 99.99 && j(inod)> 98 && i(inod)==200 && k(inod) == d_estrato_8
            ele_fisu1_1(iele) = true;
        end
    end
end
...
for iele = 1:nel
    ...
    for inod = 1:nnodel
        ...
        if j(inod) > 100 && j(inod)<102 && i(inod)==200 && k(inod) == d_estrato_8
            ele_fisu1_2(iele) = true;        
        end
    end
end
...
for iele = 1:nel
    ...
    for inod = 1:nnodel
        ...
        if j(inod) < 199.99 && j(inod)>198 && i(inod)==200 && k(inod)==d_estrato_8
            ele_fisu2_1(iele) = true;
        end
    end
end
...
for iele = 1:nel    
    ...
    for inod = 1:nnodel
        ...
        if j(inod) > 200 && j(inod)<202 && i(inod)==200 && k(inod) == d_estrato_8
            ele_fisu2_2(iele) = true;
        end
    end
end
...
for iele = 1:nel
    ...
    for inod = 1:nnodel
        ...
        if j(inod) < 299.99 && j(inod)>298 && i(inod)==200 && k(inod) == d_estrato_8
            ele_fisu3_1(iele) = true;
        end
    end
end
...
for iele = 1:nel
    ...
    for inod = 1:nnodel
        ...
        if j(inod) > 300 && j(inod)<302 && i(inod)==200 && k(inod) == d_estrato_8
            ele_fisu3_2(iele) = true;
        end
    end
end
...
desp_1 = 0.008/2; %   0.5; %   %% Definirlo según el módelo de resorte que se elija. Eso va a depender de la distancia al wellbore

despRot = 0.8*0.02/2;  %   0.6; %   %% Definirlo según el módelo de resorte que se elija. Eso va a depender de la distancia al wellbore
...
%     if itime<=25
%         Pin         = 60e6*itime*(1/25);%PresioPoralInicial + PresioPoralInicial*0; %40e6;%     %  (1e5)*itime+30e6;
%     else 
...
%     Rr = Rr + Def_Inicial;
    ...
    iteracionError=1;
        ...
        while error>5e-3
            ...
            %qlof=codigoPablo(datos,{p})
%             PressureForLeakOff(ndoftot,nelSpringFisura,nodeDofs,elements_Barra,ndofel_b,E_variable(iele),A_Barra,S,C_g,S,tita,deltaT,KC,nnod,ndof,itime,PresionPoralInicial,Desplazamientos,PorePressure,isFixedp,Q,VC)
            ...
            if iteracionError<2
                ...
                for iele = 1:nelSpringFisura
                    ke_Barra       =  k_barra(iele,nodes,elements_Barra,E_variable(iele),A_Barra);
                    [ROW, COL]     =  get_map(row(:,:,iele),col(:,:,iele));
    ...
    %% Solver con resortes
        ...
        for ispring = 1:length(gap_fisu)
            ...
            if gap_fisu(ispring)>desp_1Variable(ispring) && gap_fisu(ispring)>=gapViejo(ispring,itime) && gap_fisu(ispring)<=despRot
                ...
                jac11 = (Ediff_d1*A_Barra/le)*D_1(elements_Barra(ispring,1),2) - (Ediff_d1*A_Barra/le)*D_1(elements_Barra(ispring,2),2) + E_variable(ispring)*A_Barra/le;
                jac12 = (Ediff_d2*A_Barra/le)*D_1(elements_Barra(ispring,1),2) - ((Ediff_d2*A_Barra/le)*D_1(elements_Barra(ispring,2),2) + E_variable(ispring)*A_Barra/le);
                jac21 = -((Ediff_d1*A_Barra/le)*D_1(elements_Barra(ispring,1),2) + E_variable(ispring)*A_Barra/le) + (Ediff_d1*A_Barra/le)*D_1(elements_Barra(ispring,2),2);
                jac22 = -(Ediff_d2*A_Barra/le)*D_1(elements_Barra(ispring,1),2) + (Ediff_d2*A_Barra/le)*D_1(elements_Barra(ispring,2),2) + E_variable(ispring)*A_Barra/le;
             ...
             ke_Barra       =  k_barra(ispring,nodes,elements_Barra,E_variable(ispring),A_Barra);
             [ROW, COL]     =  get_map(row(:,:,ispring),col(:,:,ispring));
             Kbarra         =  fcn(Kbarra,sparse(ROW,COL,ke_Barra,ndoftot,ndoftot));
        end
...
addpath('animacion');
nombre2='Malla';
filename2=['F:\Corridas\GeoMec\ADINA_Mallas\Malla 1\FSI-V8 - posta\animacion\',nombre2,'.mat'];
...
for itime = 1:length(Time)
    ...
    filename=['F:\Corridas\GeoMec\ADINA_Mallas\Malla 1\FSI-V8 - posta\animacion\',nombre,'.mat'];
...
toc
%% Plot para ver resultados rápidos
