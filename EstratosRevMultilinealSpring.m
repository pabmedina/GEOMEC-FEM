...
%     while CriterioCV_Presion > 1e-4 %CriterioCV > 1e-3 && CriterioCV_desplazamientos > 1e-3  
    while Rompedor < 1 
    desp_1 = 0.018; %   0.5; %   %% Definirlo según el módelo de resorte que se elija. Eso va a depender de la distancia al wellbore
    ...
    while error>0.1
        Jacobo = sparse(ndoftot,ndoftot);
         ...
         for iele = 1:nelSpringFisura1
             ke_Barra       =  k_barra(iele,nodes,elements_Barra_fisu1,E_variable,A_Barra);
             [ROW, COL]     =  get_map(row(:,:,iele),col(:,:,iele));
...
%     A_Barra = 1000;
    ...
    for ispring = 1:length(gap_fisu1)
       E_variable = E_B;
        if gap_fisu1(ispring)>desp_1 && gap_fisu1(ispring)>=gapViejo
            if gap_fisu1(ispring)<despRot
                E_variable = (((gap_fisu1(ispring)-desp_1)*(-desp_1*E_B))/(despRot-desp_1)+desp_1*E_B)/gap_fisu1(ispring);
                Ediff_d1 = (E_B*desp_1 - (E_B*desp_1*(-gap_fisu1(ispring) + desp_1))/(desp_1 - despRot))/gap_fisu1(ispring)^2 + (E_B*desp_1)/((-gap_fisu1(ispring))*(desp_1 - despRot));
                Ediff_d2 = -(E_B*desp_1 - (E_B*desp_1*(-gap_fisu1(ispring) + desp_1))/(desp_1 - despRot))/(-gap_fisu1(ispring))^2 - (E_B*desp_1)/((-gap_fisu1(ispring))*(desp_1 - despRot));   
            else
                E_variable=0;
            end
...
%             Kbarra         =  fcn(Kbarra,sparse(ROW,COL,ke_Barra,ndoftot,ndoftot));
        ...
        if gap_fisu1(ispring)>desp_1 && gap_fisu1(ispring)>=gapViejo && gap_fisu1(ispring)<=despRot
            
            dir = nodes(elements_Barra_fisu1(ispring,2),1:2) - nodes(elements_Barra_fisu1(ispring,1),1:2);
            le = norm(dir);
            
            jac11 = (Ediff_d1*A_Barra/le)*D_1(elements_Barra_fisu1(ispring,1),2) - (Ediff_d1*A_Barra/le)*D_1(elements_Barra_fisu1(ispring,2),2) + E_variable*A_Barra/le;
            jac12 = (Ediff_d2*A_Barra/le)*D_1(elements_Barra_fisu1(ispring,1),2) - ((Ediff_d2*A_Barra/le)*D_1(elements_Barra_fisu1(ispring,2),2) + E_variable*A_Barra/le);
            jac21 = -((Ediff_d1*A_Barra/le)*D_1(elements_Barra_fisu1(ispring,1),2) + E_variable*A_Barra/le) + (Ediff_d1*A_Barra/le)*D_1(elements_Barra_fisu1(ispring,2),2);
            jac22 = -(Ediff_d2*A_Barra/le)*D_1(elements_Barra_fisu1(ispring,1),2) + (Ediff_d2*A_Barra/le)*D_1(elements_Barra_fisu1(ispring,2),2) + E_variable*A_Barra/le;
...
% %     RQincog = K_GLOBAL(isFixed,free_g)*Campos + K_GLOBAL(isFixed,isFixed)*D_P(isFixed,itime);
%     
%     CamposNuevos=Campos-JacobianoGlobal\(K_GLOBAL(free_g,free_g)*Campos-(VC(free_g) - K_GLOBAL(free_g,isFixed)*(D_P(isFixed,itime))));
%     
%     ERROR=
%     
%     %%%% Reconstrucción del vector de cargas, lo llamo RQ
