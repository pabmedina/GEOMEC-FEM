...
malla = 'M22';%'M22';%'M22';%'M333P';%'M333P';%'M3';%'M4';%'M3test';%%;%
...
toc
%% --------------------------------  Ensamblar las barras en la matriz K global. El comportamiento multilineal, por el momento, lo dejamos de lado
...
desp_1 = 0.008; %   0.5; %   %% Definirlo según el módelo de resorte que se elija. Eso va a depender de la distancia al wellbore

despRot = 0.08;  %   0.6; %   %% Definirlo según el módelo de resorte que se elija. Eso va a depender de la distancia al wellbore
...
%     if iter <= 50
    if itime<=25
        Pin         = 5e6*itime*1.02;%PresioPoralInicial + PresioPoralInicial*0; %40e6;%     % Nacho no mienta0;s : (1e5)*itime+30e6;
    else 
        Pin = 5e6;
    end
%     end
...
%     Rr = Rr + Def_Inicial;
    ...
    while error>5e-3
        Jacobo = sparse(ndoftot,ndoftot);
         ...
         for iele = 1:nelSpringFisura1
             ke_Barra       =  k_barra(iele,nodes,elements_Barra_fisu1,E_variable(iele),A_Barra);
             [ROW, COL]     =  get_map(row(:,:,iele),col(:,:,iele));
...
%        E_variable = E_B;
        ...
        if gap_fisu1(ispring)>=gapViejo(ispring,itime)
            count2(ispring)=0;
        end


        if gap_fisu1(ispring)<desp_1Variable(ispring) && gap_fisu1(ispring)>=0
            
            E_variable(ispring) = E_BB(ispring);
        ...
        if gap_fisu1(ispring)>desp_1Variable(ispring) && gap_fisu1(ispring)>=gapViejo(ispring,itime)
            if gap_fisu1(ispring)<=despRot
                E_variable(ispring) = (((gap_fisu1(ispring)-desp_1Variable(ispring))*(-desp_1Variable(ispring)*E_BB(ispring)))/(despRot-desp_1Variable(ispring))+desp_1Variable(ispring)*E_BB(ispring))/gap_fisu1(ispring);
                Ediff_d1 = (E_BB(ispring)*desp_1Variable(ispring) - (E_BB(ispring)*desp_1Variable(ispring)*(-gap_fisu1(ispring) + desp_1Variable(ispring)))/(desp_1Variable(ispring) - despRot))/gap_fisu1(ispring)^2 + (E_BB(ispring)*desp_1Variable(ispring))/((-gap_fisu1(ispring))*(desp_1Variable(ispring) - despRot));
                Ediff_d2 = -(E_BB(ispring)*desp_1Variable(ispring) - (E_BB(ispring)*desp_1Variable(ispring)*(-gap_fisu1(ispring) + desp_1Variable(ispring)))/(desp_1Variable(ispring) - despRot))/(-gap_fisu1(ispring))^2 - (E_BB(ispring)*desp_1Variable(ispring))/((-gap_fisu1(ispring))*(desp_1Variable(ispring) - despRot));   
            else
                E_variable(ispring)=0;
            end
%                 ke_Barra       =  k_barra(ispring,nodes,elements_Barra_fisu1,E_variable,A_Barra);
%                 [ROW, COL]     =  get_map(row(:,:,ispring),col(:,:,ispring));
%                 Kbarra         =  fcn(Kbarra,sparse(ROW,COL,ke_Barra,ndoftot,ndoftot));
        end
        
        if gap_fisu1(ispring)>=0 && gap_fisu1(ispring)<=gapViejo(ispring,itime)
            
            if count2(ispring)==0
                
                E_BB(ispring)=E_variable(ispring);
                desp_1Variable(ispring)=gap_fisu1(ispring);
                count2(ispring)=1;
...
%             Kbarra         =  fcn(Kbarra,sparse(ROW,COL,ke_Barra,ndoftot,ndoftot));
        ...
        if gap_fisu1(ispring)>desp_1Variable(ispring) && gap_fisu1(ispring)>=gapViejo(ispring,itime) && gap_fisu1(ispring)<=despRot
            
            dir = nodes(elements_Barra_fisu1(ispring,2),1:2) - nodes(elements_Barra_fisu1(ispring,1),1:2);
            le = norm(dir);
            
            jac11 = (Ediff_d1*A_Barra/le)*D_1(elements_Barra_fisu1(ispring,1),2) - (Ediff_d1*A_Barra/le)*D_1(elements_Barra_fisu1(ispring,2),2) + E_variable(ispring)*A_Barra/le;
            jac12 = (Ediff_d2*A_Barra/le)*D_1(elements_Barra_fisu1(ispring,1),2) - ((Ediff_d2*A_Barra/le)*D_1(elements_Barra_fisu1(ispring,2),2) + E_variable(ispring)*A_Barra/le);
            jac21 = -((Ediff_d1*A_Barra/le)*D_1(elements_Barra_fisu1(ispring,1),2) + E_variable(ispring)*A_Barra/le) + (Ediff_d1*A_Barra/le)*D_1(elements_Barra_fisu1(ispring,2),2);
            jac22 = -(Ediff_d2*A_Barra/le)*D_1(elements_Barra_fisu1(ispring,1),2) + (Ediff_d2*A_Barra/le)*D_1(elements_Barra_fisu1(ispring,2),2) + E_variable(ispring)*A_Barra/le;
...
%          for iele = 1:nelSpringFisura1
             ...
             ke_Barra       =  k_barra(ispring,nodes,elements_Barra_fisu1,E_variable(ispring),A_Barra);
             [ROW, COL]     =  get_map(row(:,:,ispring),col(:,:,ispring));
             Kbarra           =  fcn(Kbarra,sparse(ROW,COL,ke_Barra,ndoftot,ndoftot));
%          end 
    end
...
%     Campos = K_GLOBAL(free_g,free_g)\(VC(free_g) - K_GLOBAL(free_g,isFixed)*(D_P(isFixed,itime))); 
    ...
    Campos=Campos-1*(JacobianoGlobal(free_g,free_g)\(K_GLOBAL(free_g,free_g)*Campos-(VC(free_g) - K_GLOBAL(free_g,isFixed)*(D_P(isFixed,itime)))));
...
%             DIS = Desplazamientos(free,itime) - Jacobiano(free,free)\(Knew(free,free)*Desplazamientos(free,itime) - Rr(free));
%             
%             error = norm(Knew(free,free)*DIS - Rr(free))
        iteracionError=iteracionError+1;
    end
    iteracionError=iteracionError-1
    error
    gapViejo(:,itime+1)=gap_fisu1;

%     Desplazamientos(free,itime) = Desplazamientos(free,itime) + DIS;
...
%     size(nodos1_fisu1)
    ...
    for resumir=1
    if 1<2 %itime==1
        if gap_fisu1>=0 && gap_fisu1<desp_1  %%Edit NACHO!!!! LE AGREGUE EL IGUAL A CERO
...
%          min(Qneto) 
        ...
        if gap_fisu1>desp_1 && gap_fisu1<despRot
...
%              min(Qneto) 
        ...
        if gap_fisu2>desp_1 && gap_fisu2<despRot
...
%              min(Qneto) 
        ...
        if gap_fisu3>desp_1 && gap_fisu3<despRot
