function [PRESIONPAPAAAAA,QHistoricoViejo,bNew]=MOTHERFUNCTION(ro, mu, transitorio, gama, tInicial, t_final, delta_t, Pin, bomba, QMax, graficar, nod, elem8Nod, elementsBarra, P_old,nodOld,QExt,QHistoricoViejo,TBombas,nod_in,nodosDelDominioDeFisura)
% tic
...
condicion_de_borde=5;%1 primer patch test, 2 segundo patchtest, 3 tercer patchtest, 4 malla flash, El 5 es el que respeta cualquier input, el resto son debugueo con mallas particulares
malla_fractura=true;%false;%

aperturaMinima=0;%0.040000;% Para el detector automatico dominio de fisura
...
Pout=0;
PComienzo=PPorosa*1; % no se usa, ver si eliminar, quedo de comienzos del codigo para debugueo
...
BC=[nod_in
    nod_out]; %Condiciones de borde



elemNew=generadorElementosQ4(elem8Nod); %Convierte las caras de los H8 en Q4
...
selectorLogico=seleccionDeElementosDeFisura(elementsBarra,elemNew,nodosDelDominioDeFisura);   

[uno,dos]=condicionDeBorde(elementsBarra,BC,nodosDelDominioDeFisura);
...
iter=0;
% toc
[bCorr,bNew]=aperturaDeGrietaConCorreccion(nod,elementsBarra,f,f_0); %Algunos papers recomiendan corregir la apertura, esta es una de las formas
[bCorr,bOld]=aperturaDeGrietaConCorreccion(nodOld,elementsBarra,f,f_0); %Algunos papers recomiendan corregir la apertura, esta es una de las formas
...
selectorLogico3=seleccionDeElementosDeFisura(elementsBarra,elemNew,nodosDelDominioDeFisura);
aux=length(find(selectorLogico3));
for i=1:length(nodFisura)
    nodFisura2(i,:)=nodFisura(i,:)+[ 0 0 0];% tambien fue para debugueo para detectar elementos deformes
end
...
% clear elemNew
% 
% elemNew=elemFisura; %Descomentar esta y la linea de arriba para que efectivamente agarre el area de fisura
...
QExt(elementsBarra(:,1)) = QExt(elementsBarra(:,1)) + QExt(elementsBarra(:,2)); %Agarra el caudal de ambos lados de la fisura y no de un solo lado
...
%     BC(find(PAdmitida<P_old(BC)))=[];
    ...
    [uno,dos]=condicionDeBorde(elementsBarra,BC,nodosDelDominioDeFisura); %dos es donde fijo la presion, uno es donde fijo caudal entrante saliente al Volumen de Control
...
%     selectorLogico2=seleccionDeElementosDeFisura(elementsBarra,elemNew,nodosDelDominioDeFisura);
