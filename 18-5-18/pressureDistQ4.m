...
QbPoly = polyfit([pMin,(pMin+pMax)/2, pMax], [QbMax, QbMax/1.4  , QbMin],2);        % Aproximación de la curva de bomba con un polynomio



% Curva de Leak-Off

qLeakOffMax = 2*9.23E-05;
qLeakOffMin = 2*1.59E-06;
...
%% Variable necesarias para el Metodo numerico ( Resolver f(p) = Qbomba -
% Qsystema = 0) y resolvemos para la una raiz p.

p0 = pMin;                      % Valor semilla p0 
p1 = pMax;                      % Valor semilla p1

% Valores semilla de f(p0) y f(p1)
% f(p0) %
Q = ones(nDofTot,nDofNod) * - polyval(qLOPoly,p0);
Qc = Q(dofsX);
Qsys = Hcx*(Hxx\(Qc - Hxc* p0 )) + Hcc* p0;
Qbomba = polyval(QbPoly,p0)
f0 = Qsys - Qbomba

% f(p1) %
Q = ones(nDofTot,nDofNod) * - polyval(qLOPoly,p1);
Qc = Q(dofsX);
Qsys = Hcx*(Hxx\(Qc - Hxc* p1 )) + Hcc* p1;
Qbomba = polyval(QbPoly,p1)
f1 = Qsys - Qbomba


% Parametros de convergencia 
DELTA = 1e-5;                   % Tolerancia de error para la raiz
EPSILON = 1e-10;                 % Tolerancia al residuo
...
p(dofsC) = p1;
p(dofsX) = Hxx\(Qc - Hxc*p1);
Q(dofsC) = Hcx*p(dofsX) + Hcc*p1;

[a,b] = ismember(nodeDofsT(:,1),nodeDofs(:,1));


pElementsBarra(a) = p(b(a));
end

% PlotFieldonMesh(nod(:,[1 2]),elements,p);
...
% PlotFieldonMesh(nodes,elements,p)
