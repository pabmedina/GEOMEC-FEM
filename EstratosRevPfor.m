...
malla = 'M333';%'M3';%'M2';%'M333P';%'M333P';%'M3';%'M4';%'M3test';%%;%
...
[ele_fisu1_y1_1, sizeFisu1_1, ele_fisu1_y1_2, sizeFisu1_2, ele_fisu2_y2_1, sizeFisu2_1, ele_fisu2_y2_2, sizeFisu2_2, ele_fisu3_y3_1, sizeFisu3_1, ele_fisu3_y3_2, sizeFisu3_2] = areaFractura(nel,nnodel,nodes,elements); 
...
% inputNachopajero = [ nodos1_fisu1(1)
%                      nodos1_fisu2(1)
%                      nodos1_fisu3(1)];
...
nodos2_fisu1 = unique(fisura1.nodoPosicion2);             %% Idem aa, pero este me devuelve el nodo 2 de las barras, estaría ubicado en la fisura 1 del lado positivo (y+)

fisura2.auxiliar1 = reshape(ele_fisu2_y2_1,1,[])';
fisura2.nodoPosicion1 = fisura2.auxiliar1(nodes(fisura2.auxiliar1,2)>199.95);
nodos1_fisu2 = unique(fisura2.nodoPosicion1);             %% Me daría los nodos 1 de todas las barras que van en la fisura 2 del lado negativo (y-)

fisura2.auxiliar2 = reshape(ele_fisu2_y2_2,1,[])';   
fisura2.nodoPosicion2 = fisura2.auxiliar2(nodes(fisura2.auxiliar2,2)<200.05);
nodos2_fisu2 = unique(fisura2.nodoPosicion2);             %% Idem cc, pero este me devuelve el nodo 2 de las barras, estaría ubicado en la fisura 2 del lado positivo (y+)

fisura3.auxiliar1 = reshape(ele_fisu3_y3_1,1,[])';   
fisura3.nodoPosicion1 = fisura3.auxiliar1(nodes(fisura3.auxiliar1,2)>299.95);
nodos1_fisu3 = unique(fisura3.nodoPosicion1);             %% Me daría los nodos 1 de todas las barras que van en la fisura 3 del lado negativo (y-)

fisura3.auxiliar2 = reshape(ele_fisu3_y3_2,1,[])';   
fisura3.nodoPosicion2 = fisura3.auxiliar2(nodes(fisura3.auxiliar2,2)<300.05);
nodos2_fisu3 = unique(fisura3.nodoPosicion2);             %% Idem ff, pero este me devuelve el nodo 2 de las barras, estaría ubicado en la fisura 3 del lado positivo (y+)
...
nTiempo = deltaT*18;        %Decia deltaT*10 MOD NACHO!
...
% bc_poral(nodes(:,1)==0) = true;
% bc_poral(nodes(:,1)==400) = true;
% bc_poral(nodes(:,2)==0) = true;
% bc_poral(nodes(:,2)==400) = true;
% bc_poral(nodes(:,3)==0) = true;
% bc_poral(nodes(:,3)==80) = true;

Q = zeros(nNod,1);   % Creo que es el caudal

% bc_poral(nodes(:,2)==0,1) = true;
% bc_poral(nodes(:,1)==400,1) = true;
...
% PorePressure(bc_poral) = Pressure(:,itime);
...
nod_in=detectorDeCoordDeBombas(coordBombas,nodes);
...
%         Pin = 12e5*(itime - 1) + relax*12e5*(itime-1);% 12e5*(itime-1) + 12e5*(itime-1)*relax;
    ...
    [TBombas]=activacionDeBombas(delta_t*9.1*3, 3); % para secuenciar 
...
%     disp('Nacho gil... me cambiaste las conectividades')
...
%     size(nodos1_fisu1)
    ...
    for resumir=1
    if 1<2 %itime==1
        if gap_fisu1>0 && gap_fisu1<desp_1
...
%              min(Qneto) 
        ...
        if gap_fisu2>0 && gap_fisu2<desp_1
...
%              min(Qneto) 
        ...
        if gap_fisu3>0 && gap_fisu3<desp_1
...
addpath('animacion3');
nombre2='Malla';
filename2=['F:\Corridas\GeoMec\ADINA_Mallas\Malla 1\FSI-V8 - posta\animacion3\',nombre2,'.mat'];
...
for itime = 1:length(Time)
    ...
    filename=['F:\Corridas\GeoMec\ADINA_Mallas\Malla 1\FSI-V8 - posta\animacion3\',nombre,'.mat'];
